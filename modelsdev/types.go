...
// internal package, so external consumers can depend on it directly.
package modelsdev
...
// { models, providers } shape. Distinct from agentdex's index of known coding agents.
type Catalog struct {
...
// zero value, legitimate for media-generation models that carry no token limit.
type Limit struct {
...
// Benchmark is a published benchmark result for a model. Upstream it lives only
